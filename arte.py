        ...
        if artist_name:
            ...
            if not artist_data.empty:
                ...
                with st.sidebar:
                    ...
                    # Add input fields for creation date range
                    st.subheader("Creation Date Range")
                    start_creation_date = st.date_input("Start Creation Date", value=pd.to_datetime("1900-01-01"), min_value=pd.to_datetime("1700-01-01"), max_value=pd.to_datetime("2024-01-01"))
                    end_creation_date = st.date_input("End Creation Date", value=pd.to_datetime("2024-01-01"), min_value=pd.to_datetime("1700-01-01"), max_value=pd.to_datetime("2024-01-01"))
                    ...
                    selected_colours = st.multiselect("Select Colours", options=available_colours, default=available_colours)
                ...
                if start_creation_date <= end_creation_date:
                    df_filtered = df_filtered[(df_filtered['Creation_Date'] >= pd.to_datetime(start_creation_date)) & (df_filtered['Creation_Date'] <= pd.to_datetime(end_creation_date))]
                else:
                    st.error("End Creation Date must be after Start Creation Date")

                if start_date < end_date:
                    df_filtered = df_filtered[(df_filtered['Year'] >= start_date.year) & (df_filtered['Year'] <= end_date.year)]
                    ...
                    with tab1:
                        ...
                        annual_turnover = artist_data.groupby(artist_data['Auction_Date'].dt.year)['Price_USD'].sum().reset_index(name='Annual_Turnover_USD')
                        turnover_fig = px.bar(annual_turnover, x='Auction_Date', y='Annual_Turnover_USD', title='Annual Turnover (USD)')
                        st.plotly_chart(turnover_fig, use_container_width=True)

                    with tab2:
                        st.subheader("Price Analysis")
                        avg_price_per_year = df_filtered.groupby(df_filtered['Auction_Date'].dt.year)['Price_USD'].mean().reset_index(name='Average_Sale_Price_USD')
                        highest_price_per_year = df_filtered.groupby(df_filtered['Auction_Date'].dt.year)['Price_USD'].max().reset_index(name='Highest_Sale_Price_USD')

                        col1, col2 = st.columns(2)
                        with col1:
                            avg_price_fig = px.line(avg_price_per_year, x='Auction_Date', y='Average_Sale_Price_USD', title='Average Sale Price per Year')
                            st.plotly_chart(avg_price_fig, use_container_width=True)
                        with col2:
                            highest_price_fig = px.line(highest_price_per_year, x='Auction_Date', y='Highest_Sale_Price_USD', title='Highest Sale Price per Year')
                            st.plotly_chart(highest_price_fig, use_container_width=True)

                    with tab3:
                        st.subheader("Comparable Artworks")
                        if not filtered_data.empty:
                            comparable_artworks = filtered_data[['Artwork_Title', 'Auction_Date', 'Price_USD', 'Width', 'Height']].copy()
